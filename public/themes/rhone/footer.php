<a href="/" class="text-start <?php if (! $themePath): ?>mt-5 pt-5<?php endif; ?>">
    <img class="my-4" src="https://declaration.syndicat-cotesdurhone.com/images/logo_odgrhone.png" height="100px" alt="logo"/>
</a>
<div class="footer-links container">
  <ul class="footer-nav list-unstyled mx-2" role="menubar">
    <li class="nav-item d-inline-block" id="nav-item-civp" role="menuitem">
      <a class="nav-link" id="nav-link-civp" href="https://www.vins-rhone.com/fr/inter-rhone">Inter Rhône</a>
    </li>
    <li class="nav-item d-inline-block mx-2" id="nav-item-contact" role="menuitem">
      <a class="nav-link" id="nav-link-contact" href="https://www.vins-rhone.com/fr/inter-rhone#item-105">Contact</a>
    </li>
    <li class="nav-item d-inline-block mx-2" id="nav-item-mentions-legales" role="menuitem">
      <a class="nav-link" id="nav-link-mentions-legales" href="https://www.vins-rhone.com/fr/mentions-legales">Mentions légales</a>
    </li>
    <li class="nav-item d-inline-block mx-2" id="nav-item-admin" role="menuitem">
      <a class="nav-link" id="nav-link-admin" href="<?php echo Base::instance()->alias('login') ?>"><i class="fas fa-user-lock"></i> Administration</a>
    </li>
  </ul>
</div>
<hr>
<div class="footer-disclaimer">
  <div class="container">
    <a href="https://www.vins-rhone.com/fr" class="text-light" target="_blank">www.vins-rhone.com</a>
  </div>
</div>
